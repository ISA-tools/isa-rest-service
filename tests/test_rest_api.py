...
class TestIsaApi(unittest.TestCase):
    ...
    def test_convert_to_isajson(self):
        response = self.app.post(path='/isa-convert-to/isa-json', data={'file': (self.fileobj, 'BII-I-1.zip')})
        assert(response.status_code == 200)

    def test_unsupported_mimetype(self):
        response = self.app.post(path='/isa-convert-to/isa-tab', data={'file': (self.fileobj, 'BII-I-1.zip')})
